    ...
    for i, (train_id, test_id) in enumerate(kf.split(features)):
        ...
        for epoch in tqdm(range(train_epochs), total=train_epochs):
            ...
            for epoch_input_ids, epoch_attention_mask, epoch_token_type_ids, features_epoch, labels_epoch in train_dataloader:
                ...
                bert_output = bert_model(input_ids=epoch_input_ids, attention_mask=epoch_attention_mask, token_type_ids=epoch_token_type_ids).last_hidden_state
                filter_output = filter_model(bert_output)
                bio_input = torch.cat([filter_output, features_epoch.to(device)], dim=1)
        ...
        with torch.no_grad():
            for epoch_input_ids, epoch_attention_mask, epoch_token_type_ids, features_epoch, labels_epoch in test_dataloader:
                ...
                bert_output = bert_model(input_ids=epoch_input_ids, attention_mask=epoch_attention_mask, token_type_ids=epoch_token_type_ids).last_hidden_state
                filter_output = filter_model(bert_output)
                bio_input = torch.cat([filter_output, features_epoch.to(device).float()], dim=1)
...
np.save("./Result/BioNN_LSTMF_rg_error.npy", bionn_rg_error)
np.save("./Result/BioNN_LSTMF_cl_error.npy", bionn_cl_error)

biodnn_rg_error, biodnn_cl_error = validate_dl(BioDeepNN, peptides, features_x, all_mmp_y)
np.save("./Result/BioDNN_LSTMF_rg_error.npy", biodnn_rg_error)
np.save("./Result/BioDNN_LSTMF_cl_error.npy", biodnn_cl_error)

biores_rg_error, biores_cl_error = validate_dl(BioResNet, peptides, features_x, all_mmp_y)
np.save("./Result/BioRes_LSTMF_rg_error.npy", biores_rg_error)
np.save("./Result/BioRes_LSTMF_cl_error.npy", biores_cl_error)
